        ...
        if (xQueueReceive(output->m_i2sQueue, &evt, portMAX_DELAY) == pdPASS)
        {
            if (evt.type == I2S_EVENT_TX_DONE)
            {
                ...
                do
                {
                    if (availableBytes == 0)
                    {
                        // get some frames from the wave file - a frame consists of 2 samples
...
void DACOutput::start(SampleSource *sample_generator)
{
    ...
    i2s_config_t i2sConfig = {
        ...
        .communication_format = (i2s_comm_format_t)(I2S_COMM_FORMAT_STAND_PCM_LONG),
